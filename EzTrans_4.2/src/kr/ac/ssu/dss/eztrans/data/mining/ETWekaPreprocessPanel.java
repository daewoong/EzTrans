...
import kr.ac.ssu.dss.eztrans.data.mining.ETWekaExplorer.CapabilitiesFilterChangeEvent;
import kr.ac.ssu.dss.eztrans.data.mining.ETWekaExplorer.CapabilitiesFilterChangeListener;
import kr.ac.ssu.dss.eztrans.data.mining.ETWekaExplorer.ExplorerPanel;
import kr.ac.ssu.dss.eztrans.data.mining.ETWekaExplorer.LogHandler;
 ...
 */
public class ETWekaPreprocessPanel extends JPanel implements
CapabilitiesFilterChangeListener, ExplorerPanel, LogHandler {
	...
	protected JButton m_OpenFileBut = new JButton(".arff 파일 열기");

	/** Click to load base instances from a URL */
	protected JButton m_OpenURLBut = new JButton(Messages.getInstance()
			.getString("PreprocessPanel_OpenURLBut_JButton_Text"));

	/** Click to load base instances from a Database */
	protected JButton m_OpenDBBut = new JButton("DB 열기");

	/** Click to generate artificial data */
	protected JButton m_GenerateBut = new JButton(Messages.getInstance()
			.getString("PreprocessPanel_GenerateBut_JButton_Text"));

	/** Click to revert back to the last saved point */
	protected JButton m_UndoBut = new JButton("되돌리기");

	/** Click to open the current instances in a viewer */
	protected JButton m_EditBut = new JButton("편집");

	/** Click to apply filters and save the results */
	protected JButton m_SaveBut = new JButton("해당 차트 내보내기");
	...
	protected JButton m_RemoveButton = new JButton(Messages.getInstance()
			.getString("PreprocessPanel_RemoveButton_JButton_Text"));
	...
	protected ETWekaAttributeVisualizationPanel m_AttVisualizePanel = new ETWekaAttributeVisualizationPanel();
	 ...
	 */
	public ETWekaPreprocessPanel() {
		
		// Create/Configure/Connect components
		m_FilterEditor.setClassType(weka.filters.Filter.class);
		if (ETWekaExplorerDefaults.getFilter() != null)
			m_FilterEditor.setValue(ETWekaExplorerDefaults.getFilter());
		...
		m_FilterEditor.addPropertyChangeListener(new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent e) {
				...
				Capabilities currentCapabilitiesFilter = m_FilterEditor
				...
				if (filter != null && currentCapabilitiesFilter != null
					...
					currentFilterCapabilities = ((CapabilitiesHandler) filter)
							.getCapabilities();

					if (!currentFilterCapabilities
							.supportsMaybe(currentCapabilitiesFilter)
							&& !currentFilterCapabilities
							.supports(currentCapabilitiesFilter)) {
						m_ApplyFilterBut.setEnabled(false);
					}
				}
			}
		});
		m_OpenFileBut.setToolTipText("확장자가 .arff인 파일을 엽니다");
		m_OpenURLBut.setToolTipText(Messages.getInstance().getString(
				"PreprocessPanel_OpenURLBut_SetToolTipText_Text"));
		m_OpenDBBut.setToolTipText("데이터베이스로 부터 정보를 불러옵니다");
		m_GenerateBut.setToolTipText(Messages.getInstance().getString(
				"PreprocessPanel_GenerateBut_SetToolTipText_Text"));
		m_UndoBut.setToolTipText("한 수준 전으로 되돌립니다");
		m_EditBut.setToolTipText("현재 DataSet 을 편집합니다");
		m_SaveBut.setToolTipText("저장");
		m_ApplyFilterBut.setToolTipText("지정한 Filter를 적용시킵니다");
		...
		m_OpenDBBut.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				ETWekaSqlViewerDialog dialog = new ETWekaSqlViewerDialog(null);
		...
		m_SaveBut.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				// EditorPage로 보내질 해시테이블
				Hashtable<String, Vector<String>> attr = new Hashtable<String, Vector<String>>();
				// 현재 선택된 어트리뷰트의 인덱스
				int indexColumn = m_AttVisualizePanel.getM_attribIndex();
				int indexRow = m_AttVisualizePanel.getColorBox()
						.getSelectedIndex() - 1;
				Attribute column = m_AttVisualizePanel.getM_data().attribute(
						indexColumn);
				Attribute row = m_AttVisualizePanel.getM_data().attribute(
						indexRow);
				Enumeration<Instance> valueEnum = m_AttVisualizePanel
						.getM_data().enumerateInstances();
				Enumeration<String> rowEnum = row.enumerateValues();
				Enumeration<String> columnEnum = column.enumerateValues();
				Vector<String> columnNames = new Vector<String>();
				Vector<String> rowNames = new Vector<String>();
				Vector<Instance> values = new Vector<Instance>();

				// column, row 정보를 스트링으로 뽑아낸다
				if(columnEnum != null)
				{
					while (columnEnum.hasMoreElements())
						columnNames.add(columnEnum.nextElement());
				}
				if(rowEnum != null){
					while (rowEnum.hasMoreElements())
						rowNames.add(rowEnum.nextElement());
				}
				if(valueEnum != null){
				...
				int valuesMatrix[][] = new int[rowNames.size()][columnNames.size()];
				...
				for (int i = 0; i < size; i++) {
					...
					Instance ins = values.get(i);
					// System.out.println(ins.stringValue(indexColumn));
					for (int j = 0; j < columnNames.size(); j++) {
						if (ins.stringValue(indexColumn).equals(columnNames.get(j))) {
							for (int k = 0; k < rowNames.size(); k++) {
								if (ins.stringValue(indexRow).equals(
										rowNames.get(k))) {
									valuesMatrix[k][j] += 1;
									break;
								}
				...
				attr.put("column", columnNames);
				attr.put("row", rowNames);
				for(int i = 0; i < rowNames.size(); i++)
				{
					for(int j = 0; j < columnNames.size(); j++)
					{
						Vector<String> tmp = new Vector<String>();
						tmp.add(Integer.toString(valuesMatrix[i][j]));
						attr.put(columnNames.get(j) + "," + rowNames.get(i), tmp);
					}
				}
				ETEditorPage.srcMining = attr;
				ETEditorPage.flag = 10;
		...
		.setBorder(BorderFactory
				.createTitledBorder(Messages
						.getInstance()
						.getString(
								"PreprocessPanel_InstSummaryPanel_BorderFactoryCreateTitledBorder_Text")));
		...
		.setBorder(BorderFactory
				.createTitledBorder(Messages
						.getInstance()
						.getString(
								"PreprocessPanel_AttStuffHolderPanel_BorderFactoryCreateTitledBorder_Text")));
		attStuffHolderPanel.setLayout(new BorderLayout());
		attStuffHolderPanel.add(m_AttPanel, BorderLayout.CENTER);
		m_RemoveButton.setEnabled(false);
		m_RemoveButton.setToolTipText(Messages.getInstance().getString(
				"PreprocessPanel_RemoveButton_SetToolTipText_Text"));
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if (selected.length == m_Instances.numAttributes()) {
						...
						JOptionPane
						.showMessageDialog(
								ETWekaPreprocessPanel.this,
								Messages.getInstance()
								.getString(
										"PreprocessPanel_JOptionPaneShowMessageDialog_Text_First"),
										Messages.getInstance()
										.getString(
												"PreprocessPanel_JOptionPaneShowMessageDialog_Text_Second"),
												JOptionPane.ERROR_MESSAGE);
						m_Log.logMessage(Messages.getInstance().getString(
								"PreprocessPanel_Log_LogMessage_Text_First"));
						m_Log.statusMessage(Messages.getInstance().getString(
								"PreprocessPanel_Log_StatusMessage_Text_First"));
						return;
					}
					...
					.showMessageDialog(
							...
							Messages.getInstance()
							.getString(
									"PreprocessPanel_JOptionPaneShowMessageDialog_Text_Third")
									+ ex.getMessage(),
									Messages.getInstance()
									.getString(
											"PreprocessPanel_JOptionPaneShowMessageDialog_Text_Fourth"),
											JOptionPane.ERROR_MESSAGE);
					m_Log.logMessage(Messages.getInstance().getString(
							"PreprocessPanel_Log_LogMessage_Text_Second")
							+ ex.getMessage());
					m_Log.statusMessage(Messages.getInstance().getString(
							"PreprocessPanel_Log_StatusMessage_Text_Second"));
				}
			}
		});
		...
		.setBorder(BorderFactory
				.createTitledBorder(Messages
						.getInstance()
						.getString(
								"PreprocessPanel_AttSummaryPanel_BorderFactoryCreateTitledBorder_Text")));
		...
		filter.setBorder(BorderFactory
				.createTitledBorder(Messages
						.getInstance()
						.getString(
								"PreprocessPanel_Filter_BorderFactoryCreateTitledBorder_Text")));
		...
		colorBox.setToolTipText(Messages.getInstance().getString(
				"PreprocessPanel_ColorBox_SetToolTipText_Text"));
		colorBox.addItemListener(new ItemListener() {
			public void itemStateChanged(ItemEvent ie) {
				if (ie.getStateChange() == ItemEvent.SELECTED) {
					updateCapabilitiesFilter(m_FilterEditor
							.getCapabilitiesFilter());
				}
			}
		});
		final JButton visAllBut = new JButton(Messages.getInstance().getString(
				"PreprocessPanel_VisAllBut_JButton_Text"));
			...
			public void actionPerformed(ActionEvent ae) {
				if (m_Instances != null) {
					try {
						...
						as.setColoringIndex(m_AttVisualizePanel
								.getColoringIndex());
		...
		try {
			Runnable r = new Runnable() {
				public void run() {
					...
					/**
					 * m_Log.logMessage(Messages.getInstance().getString(
					 * "PreprocessPanel_SetInstances_Run_Log_LogMessage_Text_First"
					 * ) + m_Instances.relationName() +
					 * Messages.getInstance().getString(
					 * "PreprocessPanel_SetInstances_Run_Log_LogMessage_Text_Second"
					 * ) + m_Instances.numInstances() +
					 * Messages.getInstance().getString(
					 * "PreprocessPanel_SetInstances_Run_Log_LogMessage_Text_Third"
					 * ));
					 **/
					m_SaveBut.setEnabled(true);
					m_EditBut.setEnabled(true);
					m_Log.statusMessage(Messages
							.getInstance()
							.getString(
									"PreprocessPanel_SetInstances_Run_Log_StatusMessage_Text_First"));
					...
					try {
						...
						m_Instances.setClassIndex(m_AttVisualizePanel
								.getColorBox().getSelectedIndex() - 1);

						// send new ones
						if (ETWekaExplorerDefaults
								.getInitGenericObjectEditorFilter())
							getExplorer().notifyCapabilitiesFilterListener(
									Capabilities.forInstances(m_Instances));
						else
							getExplorer().notifyCapabilitiesFilterListener(
									Capabilities.forInstances(new Instances(
											m_Instances, 0)));
			 ...
			 * Messages.getInstance().getString(
			 * "PreprocessPanel_SetInstances_Run_JOptionPaneShowMessageDialog_Text_First"
			 * ) + ex, Messages.getInstance().getString(
			 * "PreprocessPanel_SetInstances_Run_JOptionPaneShowMessageDialog_Text_Second"
			 * ), JOptionPane.ERROR_MESSAGE);
			 **/
		}
	}
	...
	protected void applyFilter(final Filter filter) {
		...
		if (m_IOThread == null) {
					...
					try {
						...
						if (filter != null) {
							...
							m_Log.statusMessage(Messages
									.getInstance()
									.getString(
											"PreprocessPanel_ApplyFilter_Run_Log_StatusMessage_Text_First")
											+ filter.getClass().getName());
							String cmd = filter.getClass().getName();
							if (filter instanceof OptionHandler)
								cmd += " "
										+ Utils.joinOptions(((OptionHandler) filter)
												.getOptions());
							m_Log.logMessage(Messages
									.getInstance()
									.getString(
											"PreprocessPanel_ApplyFilter_Run_Log_LogMessage_Text_First")
											+ cmd);
							int classIndex = m_AttVisualizePanel
									.getColoringIndex();
							if ((classIndex < 0)
									&& (filter instanceof SupervisedFilter)) {
								throw new IllegalArgumentException(
										Messages.getInstance()
										.getString(
												"PreprocessPanel_ApplyFilter_Run_IllegalArgumentException_Text"));
							}
							...
							if (newInstances == null
								...
								throw new Exception(
										Messages.getInstance()
										.getString(
												"PreprocessPanel_ApplyFilter_Run_Exception_Text"));
							}
							m_Log.statusMessage(Messages
									.getInstance()
									.getString(
											"PreprocessPanel_ApplyFilter_Run_Log_StatusMessage_Text_Second"));
							addUndoPoint();
							m_AttVisualizePanel.setColoringIndex(copy
									.classIndex());
							// if class was not set before, reset it again after
							// use of filter
						...
						.showMessageDialog(
								...
								Messages.getInstance()
								.getString(
										"PreprocessPanel_ApplyFilter_Run_JOptionPaneShowMessageDialog_Text_First")
										+ ex.getMessage(),
										Messages.getInstance()
										.getString(
												"PreprocessPanel_ApplyFilter_Run_JOptionPaneShowMessageDialog_Text_Second"),
												JOptionPane.ERROR_MESSAGE);
						m_Log.logMessage(Messages
								.getInstance()
								.getString(
										"PreprocessPanel_ApplyFilter_Run_Log_LogMessage_Text_Second")
										+ ex.getMessage());
						m_Log.statusMessage(Messages
								.getInstance()
								.getString(
										"PreprocessPanel_ApplyFilter_Run_Log_StatusMessage_Text_Third"));
					}
					m_IOThread = null;
				}
			};
			...
			.showMessageDialog(
					...
					Messages.getInstance()
					.getString(
							"PreprocessPanel_ApplyFilter_Run_JOptionPaneShowMessageDialog_Text_Third"),
							Messages.getInstance()
							.getString(
									"PreprocessPanel_ApplyFilter_Run_JOptionPaneShowMessageDialog_Text_Fourth"),
	...
	public void saveWorkingInstancesToFileQ() {
		...
		if (m_IOThread == null) {
			...
			.showMessageDialog(
					...
					Messages.getInstance()
					.getString(
							"PreprocessPanel_SaveWorkingInstancesToFileQ_JOptionPaneShowMessageDialog_Text_First"),
							Messages.getInstance()
							.getString(
									"PreprocessPanel_SaveWorkingInstancesToFileQ_JOptionPaneShowMessageDialog_Text_Second"),
									JOptionPane.WARNING_MESSAGE);
		}
	}
	...
	public void saveInstancesToFile(final AbstractFileSaver saver,
		...
		if (m_IOThread == null) {
				...
				public void run() {
					try {
						m_Log.statusMessage(Messages
								.getInstance()
								.getString(
										"PreprocessPanel_SaveInstancesToFile_Run_Log_StatusMessage_Text_First"));
						...
						m_Log.statusMessage(Messages
								.getInstance()
								.getString(
										"PreprocessPanel_SaveInstancesToFile_Run_Log_StatusMessage_Text_Second"));
					} catch (Exception ex) {
						ex.printStackTrace();
			...
			.showMessageDialog(
					...
					Messages.getInstance()
					.getString(
							"PreprocessPanel_SaveInstancesToFile_Run_JOptionPaneShowMessageDialog_Text_First"),
							Messages.getInstance()
							.getString(
									"PreprocessPanel_SaveInstancesToFile_Run_JOptionPaneShowMessageDialog_Text_Second"),
									JOptionPane.WARNING_MESSAGE);
		}
	}
	...
	public void setInstancesFromFileQ() {
		...
		if (m_IOThread == null) {
			...
			if (returnVal == JFileChooser.APPROVE_OPTION) {
				...
				if (m_FileChooser.getLoader() == null) {
					...
					.showMessageDialog(
							...
							Messages.getInstance()
							.getString(
									"PreprocessPanel_SetInstancesFromFileQ_JOptionPaneShowMessageDialog_Text_First"),
									Messages.getInstance()
									.getString(
											"PreprocessPanel_SetInstancesFromFileQ_JOptionPaneShowMessageDialog_Text_Second"),
											JOptionPane.ERROR_MESSAGE);
			...
			.showMessageDialog(
					...
					Messages.getInstance()
					.getString(
							"PreprocessPanel_SetInstancesFromFileQ_JOptionPaneShowMessageDialog_Text_Third"),
							Messages.getInstance()
							.getString(
									"PreprocessPanel_SetInstancesFromFileQ_JOptionPaneShowMessageDialog_Text_Fourth"),
									JOptionPane.WARNING_MESSAGE);
		}
	}
	 ...
	 * SqlViewerDialog, then loads the instances in a background process. This
	 * is done in the IO thread, and an error message is popped up if the IO
	 * thread is busy.
	 * 
	 * @param url
	 *            the database URL
		...
		if (m_IOThread == null) {
			try {
				...
				.showMessageDialog(
						...
						Messages.getInstance()
						.getString(
								"PreprocessPanel_SetInstancesFromDBQ_JOptionPaneShowMessageDialog_Text_First")
								+ ex.getMessage(),
								Messages.getInstance()
								.getString(
										"PreprocessPanel_SetInstancesFromDBQ_JOptionPaneShowMessageDialog_Text_Second"),
										JOptionPane.ERROR_MESSAGE);
			}
			...
			.showMessageDialog(
					...
					Messages.getInstance()
					.getString(
							"PreprocessPanel_SetInstancesFromDBQ_JOptionPaneShowMessageDialog_Text_Third"),
							Messages.getInstance()
							.getString(
									"PreprocessPanel_SetInstancesFromDBQ_JOptionPaneShowMessageDialog_Text_Fourth"),
									JOptionPane.WARNING_MESSAGE);
		}
	}
	...
	public void setInstancesFromURLQ() {
		...
		if (m_IOThread == null) {
			try {
				String urlName = (String) JOptionPane
						.showInputDialog(
								...
								Messages.getInstance()
								.getString(
										"PreprocessPanel_SetInstancesFromURLQ_JOptionPaneShowMessageDialog_Text_First"),
										Messages.getInstance()
										.getString(
												"PreprocessPanel_SetInstancesFromURLQ_JOptionPaneShowMessageDialog_Text_Second"),
				...
				.showMessageDialog(
						...
						Messages.getInstance()
						.getString(
								"PreprocessPanel_SetInstancesFromURLQ_JOptionPaneShowMessageDialog_Text_Third")
								+ ex.getMessage(),
								Messages.getInstance()
								.getString(
										"PreprocessPanel_SetInstancesFromURLQ_JOptionPaneShowMessageDialog_Text_Fourth"),
										JOptionPane.ERROR_MESSAGE);
			}
		} else {
			JOptionPane
			.showMessageDialog(
					...
					Messages.getInstance()
					.getString(
							"PreprocessPanel_SetInstancesFromURLQ_JOptionPaneShowMessageDialog_Text_Fifth"),
							Messages.getInstance()
							.getString(
									"PreprocessPanel_SetInstancesFromURLQ_JOptionPaneShowMessageDialog_Text_Sixth"),
									JOptionPane.WARNING_MESSAGE);
		}
	}
		...
		if (m_IOThread == null) {
			m_IOThread = new Thread() {
				public void run() {
					try {
						// create dialog
						final ETWekaDataGeneratorPanel generatorPanel = new ETWekaDataGeneratorPanel();
						final JDialog dialog = new JDialog();
						final JButton generateButton = new JButton(
								Messages.getInstance()
								.getString(
										"PreprocessPanel_GenerateInstances_Run_GenerateButton_JButton_Text"));
						final JCheckBox showOutputCheckBox = new JCheckBox(
								Messages.getInstance()
								.getString(
										"PreprocessPanel_GenerateInstances_Run_ShowOutputCheckBox_JCheckBox_Text"));
						...
						generatorPanel.setPreferredSize(new Dimension(300,
								(int) generatorPanel.getPreferredSize()
								.getHeight()));
						...
						.setToolTipText(Messages
								.getInstance()
								.getString(
										"PreprocessPanel_GenerateInstances_Run_GenerateButton_SetToolTipText_Text"));
						generateButton.addActionListener(new ActionListener() {
							public void actionPerformed(ActionEvent evt) {
								// generate
								generatorPanel.execute();
								boolean generated = (generatorPanel
										.getInstances() != null);
								if (generated)
								...
								// display output?
								if ((generated)
										&& (showOutputCheckBox.isSelected()))
									showGeneratedInstances(generatorPanel
											.getOutput());
							}
						});
						dialog.setTitle(Messages
								.getInstance()
								.getString(
										"PreprocessPanel_GenerateInstances_Run_Dialog_Text"));
						dialog.getContentPane().add(generatorPanel,
								BorderLayout.CENTER);
						dialog.getContentPane().add(generateButton,
								BorderLayout.EAST);
						dialog.getContentPane().add(showOutputCheckBox,
								BorderLayout.SOUTH);
						dialog.pack();
			...
			.showMessageDialog(
					...
					Messages.getInstance()
					.getString(
							"PreprocessPanel_GenerateInstances_Run_JOptionPaneShowMessageDialog_Text_First"),
							Messages.getInstance()
							.getString(
									"PreprocessPanel_GenerateInstances_Run_JOptionPaneShowMessageDialog_Text_Second"),
									JOptionPane.WARNING_MESSAGE);
		}
	}
	...
	protected void showGeneratedInstances(String data) {
		...
		final JButton saveButton = new JButton(
				Messages.getInstance()
				.getString(
						"PreprocessPanel_ShowGeneratedInstances_SaveButton_JButton_Text"));
		final JButton closeButton = new JButton(
				Messages.getInstance()
				.getString(
						"PreprocessPanel_ShowGeneratedInstances_CloseButton_JButton_Text"));
		...
		.setToolTipText(Messages
				.getInstance()
				.getString(
						"PreprocessPanel_ShowGeneratedInstances_SaveButton_SetToolTipText_Text"));
				...
				if (result == JFileChooser.APPROVE_OPTION) {
					try {
						...
						.showMessageDialog(
								...
								Messages.getInstance()
								.getString(
										"PreprocessPanel_ShowGeneratedInstances_SaveButton_JOptionPaneShowMessageDialog_Text_First")
										...
										.getString(
												...
												Messages.getInstance()
												.getString(
														"PreprocessPanel_ShowGeneratedInstances_SaveButton_JOptionPaneShowMessageDialog_Text_Third"),
		...
		.setToolTipText(Messages
				.getInstance()
				.getString(
						"PreprocessPanel_ShowGeneratedInstances_CloseButton_SetToolTipText_Text"));
		...
		dialog.setTitle(Messages
				.getInstance()
				.getString(
						"PreprocessPanel_ShowGeneratedInstances_Dialog_SetToolTipText_Text"));
		dialog.getContentPane().add(new JScrollPane(textData),
				BorderLayout.CENTER);
		...
		int width = dialog.getWidth() > screen.getWidth() * 0.8 ? (int) (screen
				.getWidth() * 0.8) : dialog.getWidth();
		int height = dialog.getHeight() > screen.getHeight() * 0.8 ? (int) (screen
				.getHeight() * 0.8) : dialog.getHeight();
	...
	private void converterQuery(final File f) {
		...
		if (PropertyDialog.getParentDialog(this) != null)
			...
			pd = new PropertyDialog(PropertyDialog.getParentFrame(this),
					convEd, 100, 100);
		pd.setVisible(true);
	}
	 ...
	 * 
	 * @param cnv
	 *            the converter to apply to the input file
	 * @param f
	 *            the input file
	 */
			...
			m_IOThread = new Thread() {
				public void run() {
					try {
						...
						m_Log.statusMessage(cnv.getClass().getName()
								+ Messages
								.getInstance()
								.getString(
										"PreprocessPanel_TryConverter_Log_StatusMessage_Text")
										+ f.getName());
						JOptionPane
						.showMessageDialog(
								ETWekaPreprocessPanel.this,
								cnv.getClass().getName()
								+ Messages
								.getInstance()
								.getString(
										"PreprocessPanel_TryConverter_JOptionPaneShowMessageDialog_Text_First")
										+ f.getName()
										+ Messages
										.getInstance()
										.getString(
												...
												.getInstance()
												.getString(
														"PreprocessPanel_TryConverter_JOptionPaneShowMessageDialog_Text_Third")
														+ ex.getMessage(),
														Messages.getInstance()
														.getString(
																"PreprocessPanel_TryConverter_JOptionPaneShowMessageDialog_Text_Fourth"),
																JOptionPane.ERROR_MESSAGE);
						m_IOThread = null;
						converterQuery(f);
					}
	...
	public void setInstancesFromFile(final AbstractFileLoader loader) {
		...
		if (m_IOThread == null) {
				...
				public void run() {
					try {
						m_Log.statusMessage(Messages
								.getInstance()
								.getString(
										"PreprocessPanel_SetInstancesFromFile_Run_Log_StatusMessage_Text_First"));
						...
						m_Log.statusMessage(Messages
								.getInstance()
								.getString(
										"PreprocessPanel_SetInstancesFromFile_Run_Log_StatusMessage_Text_Second")
										+ loader.retrieveFile()
										+ Messages
										.getInstance()
										.getString(
												"PreprocessPanel_SetInstancesFromFile_Run_Log_StatusMessage_Text_Third")
												+ loader.getFileDescription()
												+ Messages
												.getInstance()
												.getString(
														"PreprocessPanel_SetInstancesFromFile_Run_Log_StatusMessage_Text_Fourth"));
						m_IOThread = null;
						if (JOptionPane
								.showOptionDialog(
										ETWekaPreprocessPanel.this,
										Messages.getInstance()
										.getString(
												"PreprocessPanel_SetInstancesFromFile_Run_JOptionPaneShowOptionDialog_Text_First")
												+ loader.retrieveFile()
												+ Messages
												.getInstance()
												.getString(
														"PreprocessPanel_SetInstancesFromFile_Run_JOptionPaneShowOptionDialog_Text_Second")
														+ loader.getFileDescription()
														+ Messages
														.getInstance()
														.getString(
																"PreprocessPanel_SetInstancesFromFile_Run_JOptionPaneShowOptionDialog_Text_Third")
																+ Messages
																.getInstance()
																.getString(
																		"PreprocessPanel_SetInstancesFromFile_Run_JOptionPaneShowOptionDialog_Text_Fourth")
																		+ ex.getMessage(),
																		Messages.getInstance()
																		.getString(
																				"PreprocessPanel_SetInstancesFromFile_Run_JOptionPaneShowOptionDialog_Text_Fifth"),
																				0,
																				JOptionPane.ERROR_MESSAGE,
																				null,
																				new String[] {
											Messages.getInstance()
											.getString(
													"PreprocessPanel_SetInstancesFromFile_Run_JOptionPaneShowOptionDialog_Text_Sixth"),
													Messages.getInstance()
													.getString(
															"PreprocessPanel_SetInstancesFromFile_Run_JOptionPaneShowOptionDialog_Text_Seventh") },
															null) == 1) {
			...
			.showMessageDialog(
					...
					Messages.getInstance()
					.getString(
							"PreprocessPanel_SetInstancesFromFile_Run_JOptionPaneShowOptionDialog_Text_Eigth"),
							Messages.getInstance()
							.getString(
									"PreprocessPanel_SetInstancesFromFile_Run_JOptionPaneShowOptionDialog_Text_Nineth"),
									JOptionPane.WARNING_MESSAGE);
		}
	}
	...
	public void setInstancesFromDB(final InstanceQuery iq) {
		if (m_IOThread == null) {
				...
				public void run() {
					...
					try {
						m_Log.statusMessage(Messages
								.getInstance()
								.getString(
										"PreprocessPanel_SetInstancesFromDB_Run_Log_StatusMessage_Text_First"));
						...
						m_Log.statusMessage(Messages
								.getInstance()
								.getString(
										"PreprocessPanel_SetInstancesFromDB_Run_Log_StatusMessage_Text_Second")
										+ m_SQLQ);
						JOptionPane
						.showMessageDialog(
								ETWekaPreprocessPanel.this,
								Messages.getInstance()
								.getString(
										"PreprocessPanel_SetInstancesFromDB_Run_JOptionPaneShowMessageDialog_Text_First")
										+ ex.getMessage(),
										Messages.getInstance()
										.getString(
												"PreprocessPanel_SetInstancesFromDB_Run_JOptionPaneShowMessageDialog_Text_Second"),
												JOptionPane.ERROR_MESSAGE);
					}
			...
			.showMessageDialog(
					...
					Messages.getInstance()
					.getString(
							"PreprocessPanel_SetInstancesFromDB_Run_JOptionPaneShowMessageDialog_Text_Third"),
							Messages.getInstance()
							.getString(
									"PreprocessPanel_SetInstancesFromDB_Run_JOptionPaneShowMessageDialog_Text_Fourth"),
									JOptionPane.WARNING_MESSAGE);
		}
	}
	...
	public void setInstancesFromURL(final URL u) {
		...
		if (m_IOThread == null) {
				...
				public void run() {
					...
					try {
						m_Log.statusMessage(Messages
								.getInstance()
								.getString(
										"PreprocessPanel_SetInstancesFromURL_Run_Log_StatusMessage_Text_First"));
						AbstractFileLoader loader = ConverterUtils
								.getURLLoaderForFile(u.toString());
						if (loader == null)
							throw new Exception(
									Messages.getInstance()
									.getString(
											"PreprocessPanel_SetInstancesFromURL_Run_Exception_Text_First")
											+ u);
						...
						m_Log.statusMessage(Messages
								.getInstance()
								.getString(
										"PreprocessPanel_SetInstancesFromURL_Run_Log_StatusMessage_Text_Second")
										+ u);
						JOptionPane
						.showMessageDialog(
								ETWekaPreprocessPanel.this,
								Messages.getInstance()
								.getString(
										"PreprocessPanel_SetInstancesFromDB_Run_JOptionPaneShowMessageDialog_Text_Fifth")
										+ u
										+ Messages
										.getInstance()
										.getString(
												...
												Messages.getInstance()
												.getString(
														"PreprocessPanel_SetInstancesFromDB_Run_JOptionPaneShowMessageDialog_Text_Seventh"),
														JOptionPane.ERROR_MESSAGE);
					}
			...
			.showMessageDialog(
					...
					Messages.getInstance()
					.getString(
							"PreprocessPanel_SetInstancesFromDB_Run_JOptionPaneShowMessageDialog_Text_Eighth"),
							Messages.getInstance()
							.getString(
									"PreprocessPanel_SetInstancesFromDB_Run_JOptionPaneShowMessageDialog_Text_Nineth"),
									JOptionPane.WARNING_MESSAGE);
		}
	}
	...
	public void addUndoPoint() throws Exception {
		...
		if (m_Instances != null) {
			...
			File tempFile = File.createTempFile("weka",
	...
	public void undo() {
		...
		if (m_tempUndoFiles[m_tempUndoIndex] != null) {
			...
			AbstractFileLoader loader = ConverterUtils
					.getLoaderForFile(m_tempUndoFiles[m_tempUndoIndex]);
			try {
				...
				.showMessageDialog(
						...
						Messages.getInstance()
						.getString(
								"PreprocessPanel_Undo_JOptionPaneShowMessageDialog_Text_First")
								+ e.toString(),
								Messages.getInstance()
								.getString(
										"PreprocessPanel_Undo_JOptionPaneShowMessageDialog_Text_Second"),
										JOptionPane.ERROR_MESSAGE);
			}
	...
	protected void updateCapabilitiesFilter(Capabilities filter) {
		...
		if (!ETWekaExplorerDefaults.getInitGenericObjectEditorFilter())
			tempInst = new Instances(m_Instances, 0);
		else
			tempInst = new Instances(m_Instances);
		tempInst.setClassIndex(m_AttVisualizePanel.getColorBox()
				.getSelectedIndex() - 1);
		...
		if (currentFilter != null && currentCapabilitiesFilter != null
			...
			currentFilterCapabilities = ((CapabilitiesHandler) currentFilter)
					.getCapabilities();
		...
		System.out.println(m_AttVisualizePanel.getM_data());
		System.out.println("m_as---------------------");
		System.out.println(m_AttVisualizePanel.getM_as());
		System.out.println("m_attribIndex---------------------");
		System.out
		.println(m_AttVisualizePanel.getColorBox().getSelectedIndex());
		// System.out.println(m_AttVisualizePanel.getM_attribIndex());
		// System.out.println("m_classIndex---------------------");
		// System.out.println(m_AttVisualizePanel.getM_classIndex());
		// System.out.println("m_maxValue---------------------");
		// System.out.println(m_AttVisualizePanel.getM_maxValue());
		// System.out.println("m_histBarCounts---------------------");
		// System.out.println(m_AttVisualizePanel.getM_histBarCounts());
		// System.out.println("m_barRange---------------------");
		// System.out.println(m_AttVisualizePanel.getM_barRange());
		// System.out.println("m_colorAttrib---------------------");
		// System.out.println(m_AttVisualizePanel.getM_colorAttrib());
		// System.out.println("m_colorList---------------------");
		// System.out.println(m_AttVisualizePanel.getM_colorList());
	}
	...
	public String getTabTitle() {
		return Messages.getInstance()
				.getString("PreprocessPanel_GetTitle_Text");
	}
	...
	public String getTabTitleToolTip() {
		return Messages.getInstance().getString(
				"PreprocessPanel_GetTabTitleToolTip_Text");
	}
	...
	public static void main(String[] args) {
		...
		try {
			final JFrame jf = new JFrame(Messages.getInstance().getString(
					"PreprocessPanel_Main_JFRame_Text"));
			jf.getContentPane().setLayout(new BorderLayout());
			final ETWekaPreprocessPanel sp = new ETWekaPreprocessPanel();
