 ...
 */
public class ETWekaAttributeVisualizationPanel extends PrintablePanel {
	...
	public AttributeStats getM_as() {
		return m_as;
	}

	public int getM_attribIndex() {
		return m_attribIndex;
	}

	public int getM_maxValue() {
		return m_maxValue;
	}

	public int[] getM_histBarCounts() {
		return m_histBarCounts;
	}

	public SparseInstance[] getM_histBarClassCounts() {
		return m_histBarClassCounts;
	}

	public double getM_barRange() {
		return m_barRange;
	}
	...
	public Thread getM_hc() {
		return m_hc;
	}

	public boolean isM_doneCurrentAttribute() {
		return m_doneCurrentAttribute;
	}

	public boolean isM_displayCurrentAttribute() {
		return m_displayCurrentAttribute;
	}

	public JComboBox getM_colorAttrib() {
		return m_colorAttrib;
	}

	public FontMetrics getM_fm() {
		return m_fm;
	}

	public Integer getM_locker() {
		return m_locker;
	}

	public FastVector getM_colorList() {
		return m_colorList;
	}
	 ...
	 * attribute it is simply the maximum value present in the attribute (given
	 * by m_as.numericStats.max). It is used to calculate the ratio of the
	 * height of the bars with respect to the height of the display area.
	 */
	 ...
	 * nominal values in the current attribute, with each field of the array
	 * being equal to the count of each nominal that it represents (the count of
	 * ith nominal value of an attribute is given by m_as.nominalCounts[i]).
	 * Whereas, in case of histograms (and current attribute being numeric) the
	 * width of its intervals is calculated by Scott's(1979) method: <br>
	 * intervalWidth = Max(1, 3.49*Std.Dev*numInstances^(1/3)) And the number of
	 * intervals by: <br>
	 * intervals = max(1, Math.round(Range/intervalWidth); Then each field of
	 * this array contains the number of values of the current attribute that
	 * fall in the histogram interval that it represents. <br>
	 ...
	 * the bars in a barplot or a histogram. For nominal attributes the format
	 * is: <br>
	 ...
	 * this class. There are two to three threads in this class, AWT paint
	 * thread which is handled differently in paintComponent() which checks on
	 * m_threadRun to determine if it can perform full paint or not, the second
	 * thread is the main execution thread and the third is the one represented
	 * by m_hc which we start when we want to calculate the internal fields for
	 * a bar plot or a histogram.
	 */
	...
	private static final Color[] m_defaultColors = { Color.blue, Color.red,
			Color.cyan, new Color(75, 123, 130), Color.pink, Color.green,
			Color.orange, new Color(255, 0, 255), new Color(255, 0, 0),
			new Color(0, 255, 0), };
	 ...
	 * combo box.
	 */
	public ETWekaAttributeVisualizationPanel() {
		this(false);
	}
	 ...
	 * Constructor.
	 * 
	 * @param showColouringOption
	 *            - should be true if the class selection combo box is to be
	 *            displayed with the histogram/barplot, or false otherwise. P.S:
	 *            the combo box is always created it just won't be shown if
	 *            showColouringOption is false.
	 */
	public ETWekaAttributeVisualizationPanel(boolean showColouringOption) {
		...
		if (m_colorAttrib != null) {
			...
			m_colorAttrib
					.addItem(Messages
							.getInstance()
							.getString(
									"AttributeVisualizationPanel_SetInstances_ColorAttrib_Text_First"));
			for (int i = 0; i < m_data.numAttributes(); i++) {
				...
				case Attribute.NOMINAL:
					type = Messages
							.getInstance()
							.getString(
									"AttributeVisualizationPanel_SetInstances_AttributeNOMINAL_Text");
					break;
				case Attribute.NUMERIC:
					type = Messages
							.getInstance()
							.getString(
									"AttributeVisualizationPanel_SetInstances_AttributeNUMERIC_Text");
					break;
				case Attribute.STRING:
					type = Messages
							.getInstance()
							.getString(
									"AttributeVisualizationPanel_SetInstances_AttributeSTRING_Text");
					break;
				case Attribute.DATE:
					type = Messages
							.getInstance()
				...
				case Attribute.RELATIONAL:
					type = Messages
							.getInstance()
							.getString(
									"AttributeVisualizationPanel_SetInstances_AttributeRELATIONAL_Text");
					break;
				default:
					type = Messages
							.getInstance()
				...
				m_colorAttrib
						.addItem(new String(
								Messages.getInstance()
										.getString(
												"AttributeVisualizationPanel_SetInstances_ColorAttrib_Text_Second")
	...
	/**
	 * Recalculates the barplot or histogram to display, required usually when
	 * the attribute is changed or the component is resized.
	 */
	...
	private class BarCalc extends Thread {
		...
		public void run() {
			synchronized (m_locker) {
				...
				// of the panel is less than the number of values for this
				// attribute
				if (m_data.attribute(m_attribIndex).numValues() > m_panelWidth) {
					...
					ETWekaAttributeVisualizationPanel.this.repaint();
					return;
				}
				...
				if ((m_classIndex >= 0)
					...
					histClassCounts = new SparseInstance[m_data.attribute(
							m_attribIndex).numValues()];
					/*
					 * int histClassCounts[][]; histClassCounts=new
					 * int[m_data.attribute(m_attribIndex).numValues()]
					 * [m_data.attribute(m_classIndex).numValues()+1];
					 */

					if (m_as.nominalCounts.length > 0) {
						m_maxValue = m_as.nominalCounts[0];
						for (int i = 0; i < m_data.attribute(m_attribIndex)
								.numValues(); i++) {
					...
					for (int i = m_colorList.size(); i < m_data.attribute(
							m_classIndex).numValues() + 1; i++) {
					...
					for (int k = 0; k < m_data.numInstances(); k++) {
						...
						if (!m_data.instance(k).isMissing(m_attribIndex)) {
							...
							if (m_data.instance(k).value(m_attribIndex) != tempAttValueIndex) {
								if (tempClassCounts != null) {
									...
									SparseInstance tempS = new SparseInstance(
											1.0, nonZeroVals, nonZeroIndices,
											tempClassCounts.length);
									histClassCounts[tempAttValueIndex] = tempS;
								}

								tempClassCounts = new double[m_data.attribute(
										m_classIndex).numValues() + 1];
								tempAttValueIndex = (int) m_data.instance(k)
										.value(m_attribIndex);

								/*
								 * histClassCounts[(int)m_data.instance(k).value(
								 * m_attribIndex)] = new
								 * double[m_data.attribute(
								 * m_classIndex).numValues()+1];
								 */
							}
							if (m_data.instance(k).isMissing(m_classIndex)) {
								 ...
								 */
								tempClassCounts[0] += m_data.instance(k)
										.weight();
							} else {
								tempClassCounts[(int) m_data.instance(k).value(
										m_classIndex) + 1] += m_data
										.instance(k).weight();

								/*
								 * histClassCounts[(int)m_data.instance(k).value(
								 * m_attribIndex)]
								 * [(int)m_data.instance(k).value
								 * (m_classIndex)+1] +=
								 * m_data.instance(k).weight();
								 */
							}
					...
					if (tempClassCounts != null) {
						// set up the sparse instance for the previous bar (if
						// any)
						int numNonZero = 0;
						for (int z = 0; z < tempClassCounts.length; z++) {
							if (tempClassCounts[z] > 0) {
						...
						SparseInstance tempS = new SparseInstance(1.0,
								nonZeroVals, nonZeroIndices,
								tempClassCounts.length);
						histClassCounts[tempAttValueIndex] = tempS;
					}
					 ...
					 * for(int k=0; k<m_data.numInstances(); k++) {
					 * //System.out.println("attrib: "+ //
					 * m_data.instance(k).value(m_attribIndex)+ // " class: "+
					 * // m_data.instance(k).value(m_classIndex));
					 * if(!m_data.instance(k).isMissing(m_attribIndex))
					 * if(m_data.instance(k).isMissing(m_classIndex))
					 * histClassCounts
					 * [(int)m_data.instance(k).value(m_attribIndex)] [0]++;
					 * else
					...
					int histCounts[];
					histCounts = new int[m_data.attribute(m_attribIndex)
							.numValues()];
					...
					for (int k = 0; k < m_data.numInstances(); k++) {
						if (!m_data.instance(k).isMissing(m_attribIndex))
							histCounts[(int) m_data.instance(k).value(
									m_attribIndex)]++;
					}
					m_threadRun = false;
	...
	private class HistCalc extends Thread {
		public void run() {
			synchronized (m_locker) {
				if ((m_classIndex >= 0)
					...
					// This uses the M.P.Wand's method to calculate the
					// histogram's
					// interval width. See
					// "Data-Based Choice of Histogram Bin Width", in
					// The American Statistician, Vol. 51, No. 1, Feb., 1997,
					// pp. 59-64.
					// intervalWidth = Math.pow(6D/( -psi(2,
					// g21())*m_data.numInstances()),
					// 1/3D );

					// This uses the Scott's method to calculate the histogram's
					// interval
					// width. See "On optimal and data-based histograms".
					// See Biometrika, 66, 605-610 OR see the same paper
					// mentioned above.
					...
					// intervals=0 and =NAN that can happen if respectively all
					// the numeric
					// values are the same or the interval width is evaluated to
					// zero.
					intervals = Math
							.max(1,
									(int) Math
											.round((m_as.numericStats.max - m_as.numericStats.min)
													/ intervalWidth));
					...
					// The number 4 below actually represents a padding of 3
					// pixels on
					// each side of the histogram, and is also reflected in
					// other parts of
					// the code in the shape of numerical constants like "6"
					// here.
					if (intervals > ETWekaAttributeVisualizationPanel.this.getWidth()) {
						intervals = ETWekaAttributeVisualizationPanel.this.getWidth() - 6;
						if (intervals < 1)// if width is too small then use 1
											// and forget padding
							intervals = 1;
					}
					int histClassCounts[][] = new int[intervals][m_data
							.attribute(m_classIndex).numValues() + 1];
					...
					for (int k = 0; k < m_data.numInstances(); k++) {
						int t = 0; // This holds the interval that the attibute
									// value of the
						...
						try {
							if (!m_data.instance(k).isMissing(m_attribIndex)) {
								...
								t = (int) Math
										.ceil((float) ((m_data.instance(k)
												.value(m_attribIndex) - m_as.numericStats.min) / barRange));
								if (t == 0) {
									if (m_data.instance(k).isMissing(
											m_classIndex))
										histClassCounts[t][0]++;
									else
										histClassCounts[t][(int) m_data
												.instance(k)
												.value(m_classIndex) + 1]++;
									...
									if (m_data.instance(k).isMissing(
										...
										histClassCounts[t - 1][(int) m_data
												.instance(k)
												.value(m_classIndex) + 1]++;
							...
							System.out
									.println("t:"
											...
											+ " value:"
											+ m_data.instance(k).value(
													m_attribIndex)
											+ " min:"
											+ m_as.numericStats.min
											+ " sumResult:"
											+ (m_data.instance(k).value(
													m_attribIndex) - m_as.numericStats.min)
											+ " divideResult:"
											+ (float) ((m_data.instance(k)
													.value(m_attribIndex) - m_as.numericStats.min) / barRange)
											+ " finalResult:"
											+ Math.ceil((float) ((m_data
													.instance(k).value(
				...
				} else { // else if the class attribute is numeric or the class
							// is not set
					...
					for (int k = 0; k < m_data.numInstances(); k++) {
						int t = 0; // This holds the interval to which the
									// current attribute's
									// value of this particular instance k
									// belongs to.
						...
						try {
							// 1. see footnote at the end of this file
							t = (int) Math
									.ceil((float) ((m_data.instance(k).value(
											m_attribIndex) - m_as.numericStats.min) / barRange));
							if (t == 0) {
								histCounts[t]++;
								if (histCounts[t] > m_maxValue)
							...
							System.out
									.println(Messages
											.getInstance()
											.getString(
													"AttributeVisualizationPanel_HistCalc_Run_ArrayIndexOutOfBoundsException_Text_First")
											+ (t)
											+ Messages
													.getInstance()
													.getString(
															"AttributeVisualizationPanel_HistCalc_Run_ArrayIndexOutOfBoundsException_Text_Second")
											+ barRange
											+ Messages
													.getInstance()
													.getString(
															"AttributeVisualizationPanel_HistCalc_Run_ArrayIndexOutOfBoundsException_Text_Third")
											+ histCounts.length
											+ Messages
													.getInstance()
													.getString(
															"AttributeVisualizationPanel_HistCalc_Run_ArrayIndexOutOfBoundsException_Text_Fourth")
											+ m_data.instance(k).value(
													m_attribIndex)
											+ Messages
													.getInstance()
													.getString(
															"AttributeVisualizationPanel_HistCalc_Run_ArrayIndexOutOfBoundsException_Text_Fifth")
											+ m_as.numericStats.min
											+ Messages
													.getInstance()
													.getString(
															"AttributeVisualizationPanel_HistCalc_Run_ArrayIndexOutOfBoundsException_Text_Sixth")
											+ (m_data.instance(k).value(
													m_attribIndex) - m_as.numericStats.min)
											+ Messages
													.getInstance()
													.getString(
															"AttributeVisualizationPanel_HistCalc_Run_ArrayIndexOutOfBoundsException_Text_Seventh")
											+ (float) ((m_data.instance(k)
													.value(m_attribIndex) - m_as.numericStats.min) / barRange)
											+ Messages
													.getInstance()
													.getString(
															"AttributeVisualizationPanel_HistCalc_Run_ArrayIndexOutOfBoundsException_Text_Eighth")
											+ Math.ceil((float) ((m_data
													.instance(k).value(
															m_attribIndex) - m_as.numericStats.min) / barRange)));
						}
		 ...
		 * some problem with it. It always comes up -ve value which is raised to
		 * the power 1/3 and gives an NAN. private static final int M=400;
		 * private double psi(int r, double g) { double val;
		 * 
		 * double sum=0.0; for(int i=0; i<M; i++) { double valCjKj=0.0; for(int
		 * j=0; j<M; j++) { valCjKj += c(j) * k(r, j-i, g); } sum +=
		 * valCjKj*c(i); }
		 * 
		 * val = Math.pow(m_data.numInstances(), -2) * sum;
		 * //System.out.println("psi returns: "+val); return val; } private
		 * double g21() { double val;
		 * 
		 * val = Math.pow(2 / ( Math.sqrt(2D*Math.PI)*psi(4, g22()) *
		 * m_data.numInstances() ), 1/5D) * Math.sqrt(2) *
		 * m_as.numericStats.stdDev; //System.out.println("g21 returns: "+val);
		 * return val; } private double g22() { double val;
		 * 
		 ...
		 * if(!m_data.instance(i).isMissing(m_attribIndex)) val += Math.max( 0,
		 * ( 1 - Math.abs( Math.pow(sigma,
		 * -1)*(m_data.instance(i).value(m_attribIndex) - j) ) ) ); }
		 * //System.out.println("c returns: "+val); return val; } private double
		 * k(int r, int j, double g) { double val; double sigma =
		 * (m_as.numericStats.max - m_as.numericStats.min)/(M-1);
		 * //System.out.println("Before calling L we have");
		 * //System.out.println
		 * ("Max: "+m_as.numericStats.max+" Min: "+m_as.numericStats.min+"\n"+
		 * // "r: "+r+" j: "+j+" g: "+g); val = Math.pow( g, -r-1) *
		 * L(sigma*j/g); //System.out.println("k returns: "+val); return val; }
		 * private double L(double x) { double val;
		 * 
	...
	/**
	 * Returns "&lt;nominal value&gt; [&lt;nominal value count&gt;]" if
	 * displaying a bar plot and mouse is on some bar. If displaying histogram
	 * then it <li>returns "count &lt;br&gt; [&lt;bars Range&gt;]" if mouse is
	 * on the first bar.</li> <li>returns
	 * "count &lt;br&gt; (&lt;bar's Range&gt;]" if mouse is on some bar other
	 * than the first one.</li> Otherwise it returns ""
	 * 
	 * @param ev
	 *            The mouse event
	 */
	public String getToolTipText(MouseEvent ev) {
		...
		if (m_as != null && m_as.nominalCounts != null) { // if current attrib
															// is nominal

			float intervalWidth = this.getWidth()
					/ (float) m_as.nominalCounts.length;
			float heightRatio;
			int barWidth, x = 0, y = 0;

			// if intervalWidth is at least six then bar width is 80% of
			// intervalwidth
			if (intervalWidth > 5) // the rest is padding
				barWidth = (int) Math.floor(intervalWidth * 0.8F);
			else
				barWidth = 1; // Otherwise barwidth is 1 & padding would be at
								// least 1.

			// initializing x to maximum of 1 or 10% of interval width (i.e.
			// half of
			// the padding which is 20% of interval width, as there is 10% on
			// each
			...
			// Adding to x the appropriate value so that it points to the 1st
			// bar of
			// our "centered" barplot. If subtracting barplots width from panel
			// width
			// gives <=2 then the barplot is not centered.
			if (this.getWidth()
					- (m_as.nominalCounts.length * barWidth + (int) ((Math
							.floor(intervalWidth * 0.2F)) < 1 ? 1 : (Math
							.floor(intervalWidth * 0.2F)))
							* m_as.nominalCounts.length) > 2) {

				// The following amounts to adding to x the half of the area
				// left after
				// subtracting from the components width the width of the whole
				// barplot
				// (i.e. width of all the bars plus the width of all the bar
				// paddings,
				// thereby equaling to the whole barplot), since our barplot is
				// centered.
				x += (this.getWidth() - (m_as.nominalCounts.length * barWidth + (int) ((Math
						.floor(intervalWidth * 0.2F)) < 1 ? 1 : (Math
						.floor(intervalWidth * 0.2F)))
						* m_as.nominalCounts.length)) / 2;
			}
			...
			for (int i = 0; i < m_as.nominalCounts.length; i++) {
				...
				// if our mouse is on a bar then return the count of this bar in
				// our
				// barplot
				if (ev.getX() >= x
						&& ev.getX() <= x + barWidth
						&& ev.getY() >= this.getHeight()
								- Math.round(m_as.nominalCounts[i]
										* heightRatio))
					return (m_data.attribute(m_attribIndex).value(i) + " ["
							+ m_as.nominalCounts[i] + "]");
				// otherwise advance x to next bar and check that. Add barwidth
				// to x
			...
			if ((m_classIndex >= 0)
				...
				// the barwidth is 1 if after removing the padding its width is
				// less
				...
				// initializing x to 3 adding appropriate value to make it point
				// to the
				// start of the 1st bar of our "centered" histogram.
				x = 3;
				if ((this.getWidth() - (x + m_histBarClassCounts.length
						* barWidth)) > 5)
				...
				if (ev.getX() - x >= 0) {
					// The temp holds the index of the current interval that we
					// are looking
					// at
					int temp = (int) ((ev.getX() - x) / (barWidth + 0.0000000001));
					if (temp == 0) { // handle the special case temp==0. see
										// footnote 1
						int sum = 0;
						for (int k = 0; k < m_histBarClassCounts[0].numValues(); k++)
							sum += m_histBarClassCounts[0].valueSparse(k);
						// return the count of the interval mouse is pointing to
						// plus
						...
						return (Messages
								...
								+ Utils.doubleToString(bar + m_barRange * temp,
										3)
								+ ", "
								+ Utils.doubleToString((bar + m_barRange
										* (temp + 1)), 3) + Messages
								.getInstance()
								.getString(
										"AttributeVisualizationPanel_GetToolTipText_Text_Fourth"));
					} else if (temp < m_histBarClassCounts.length) { // handle
																		// case
																		// temp!=0
						int sum = 0;
						for (int k = 0; k < m_histBarClassCounts[temp]
								.numValues(); k++)
				...
				if ((this.getWidth() - (x + m_histBarCounts.length * barWidth)) > 5)
					x += (this.getWidth() - (x + m_histBarCounts.length
							* barWidth)) / 2;
				...
				if (ev.getX() - x >= 0) {
					// Temp holds the index of the current bar we are looking
					// at.
					int temp = (int) ((ev.getX() - x) / (barWidth + 0.0000000001));

					// return interval count as well as its range
					if (temp == 0) // handle special case temp==0. see footnote
									// 1.
						return (Messages
								.getInstance()
								.getString(
					...
					else if (temp < m_histBarCounts.length) // handle case
															// temp!=0
						return (Messages
								.getInstance()
								.getString(
	...
	public void paintComponent(Graphics g) {
		...
		if (m_as != null) { // If calculations have been done and
							// histogram/barplot
			...
			if (m_threadRun == false && m_displayCurrentAttribute) { // calculation
				...
				if (m_as.nominalCounts != null
					...
					// if the class attribute is set and is nominal then draw
					// coloured
					...
					if ((m_classIndex >= 0)
						...
						// Barwidth is 80% of interval width.The remaining 20%
						// is padding,
						// 10% on each side of the bar. If interval width is
						// less then 5 the
						// 20% of that value is less than 1, in that case we use
						// bar width of
						...
						// initializing x to 10% of interval width or to 1 if
						// 10% is <1. This
						// is essentially the LHS padding of the 1st bar.
						x = x
								+ (int) ((Math.floor(intervalWidth * 0.1F)) < 1 ? 1
										: (Math.floor(intervalWidth * 0.1F)));

						// Add appropriate value to x so that it starts at the
						// 1st bar of
						// a "centered" barplot.
						if (this.getWidth()
								- (m_histBarClassCounts.length * barWidth + (int) ((Math
										.floor(intervalWidth * 0.2F)) < 1 ? 1
										: (Math.floor(intervalWidth * 0.2F)))
										* m_histBarClassCounts.length) > 2) {
							// We take the width of all the bars and all the
							// paddings (20%
							// of interval width), and subtract it from the
							// width of the panel
							// to get the extra space that would be left after
							// drawing. We
							// divide that space by 2 to get its mid-point and
							// add that to our
							// x, thus making the whole bar plot drawn centered
							// in our
							// component.
							x += (this.getWidth() - (m_histBarClassCounts.length
									* barWidth + (int) ((Math
									.floor(intervalWidth * 0.2F)) < 1 ? 1
									: (Math.floor(intervalWidth * 0.2F)))
									* m_histBarClassCounts.length)) / 2;
						...
						// this holds the count of the bar and will be
						// calculated by adding
						// up the counts of individual subbars. It is displayed
						// at the top
						// of each bar.
						int sum = 0;
						for (int i = 0; i < m_histBarClassCounts.length; i++) {
							// calculating the proportion of the components
							// height compared to
							// the maxvalue in our attribute, also taking into
							// account the
							// height of font to display bars count and the
							// height of the class
							// ComboBox.
							heightRatio = (this.getHeight()
									- (float) m_fm.getHeight() - buttonHeight)
									/ m_maxValue;
							y = this.getHeight();
							if (m_histBarClassCounts[i] != null) {
								for (int j = 0; j < m_histBarClassCounts[i]
										.numAttributes(); j++) {
									sum = (int) (sum + m_histBarClassCounts[i]
											.value(j));
									y = (int) (y - Math
											.round(m_histBarClassCounts[i]
													.value(j) * heightRatio));
									// selecting the colour corresponding to the
									// current class.
							...
							// drawing the bar count at the top of the bar if it
							// is less than
							// interval width. draw it 1px up to avoid touching
							// the bar.
							if (m_fm.stringWidth(Integer.toString(sum)) < intervalWidth)
								g.drawString(Integer.toString(sum), x, y - 1);
							// advancing x to the next bar by adding bar width
							// and padding
							// of both the bars (i.e. RHS padding of the bar
							// just drawn and LHS
					...
					// else if class attribute is numeric or not set then draw
					// black bars.
					else {
						intervalWidth = (this.getWidth() / (float) m_histBarCounts.length);

						// same as in the case of nominal class (see inside of
						// if stmt
						...
						if (this.getWidth()
							...
							x += (this.getWidth() - (m_histBarCounts.length
									* barWidth + (int) ((Math
									.floor(intervalWidth * 0.2F)) < 1 ? 1
									: (Math.floor(intervalWidth * 0.2F)))
									* m_histBarCounts.length)) / 2;
						...
						for (int i = 0; i < m_histBarCounts.length; i++) {
							// calculating the proportion of the height of the
							// component
							...
							heightRatio = (this.getHeight()
									...
									/ m_maxValue;
							y = this.getHeight()
									- Math.round(m_histBarCounts[i]
											* heightRatio);
							g.fillRect(x, y, barWidth, Math
									.round(m_histBarCounts[i] * heightRatio));
							// draw the bar count if it's width is smaller than
							// intervalWidth.
							// draw it 1px above to avoid touching the bar.
							if (m_fm.stringWidth(Integer
									.toString(m_histBarCounts[i])) < intervalWidth)
								g.drawString(
										Integer.toString(m_histBarCounts[i]),
										x, y - 1);
							// Advance x to the next bar by adding bar-width and
							// padding
							// of the bars (RHS padding of current bar & LHS
							// padding of next
							// bar).
							x = x
									+ barWidth
					...
					// if the current attribute is numeric then draw a
					// histogram.
				else if (m_as.numericStats != null
						&& (m_histBarClassCounts != null || m_histBarCounts != null)) {
					...
					// If the class attribute is set and is not numeric then
					// draw coloured
					// subbars for the histogram bars
					if ((m_classIndex >= 0)
							&& (m_data.attribute(m_classIndex).isNominal())) {

						// There is a padding of 3px on each side of the
						// histogram.
						barWidth = ((this.getWidth() - 6) / m_histBarClassCounts.length) < 1 ? 1
								: ((this.getWidth() - 6) / m_histBarClassCounts.length);

						// initializing x to start at the start of the 1st bar
						// after padding.
						x = 3;
						// Adding appropriate value to x to account for a
						// "centered"
						// histogram
						if ((this.getWidth() - (x + m_histBarClassCounts.length
								* barWidth)) > 5) {
							// We take the current value of x (histogram's RHS
							// padding) and add
							// the barWidths of all the bars to it to us the
							// size of
							// our histogram. We subtract that from the width of
							// the panel
							// giving us the extra space that would be left if
							// the histogram is
							// drawn and divide that by 2 to get the midpoint of
							// that extra
							// space. That space is then added to our x, hence
							// making the
							// histogram centered.
							x += (this.getWidth() - (x + m_histBarClassCounts.length
									* barWidth)) / 2;
						...
						for (int i = 0; i < m_histBarClassCounts.length; i++) {
							if (m_histBarClassCounts[i] != null) {
								// Calculating height ratio. Leave space of 19
								// for an axis line at
								// the bottom
								heightRatio = (this.getHeight()
										...
										/ m_maxValue;
								y = this.getHeight() - 19;
								// This would hold the count of the bar (sum of
								// sub-bars).
								int sum = 0;
								for (int j = 0; j < m_histBarClassCounts[i]
										.numValues(); j++) {
									...
									// selecting the color corresponding to our
									// class
									g.setColor((Color) m_colorList
											.elementAt(m_histBarClassCounts[i]
													.index(j)));
									// drawing the bar if its width is greater
									// than 1
									if (barWidth > 1)
										g.fillRect(x, y, barWidth, (int) Math
												.round(m_histBarClassCounts[i]
									...
									else if ((m_histBarClassCounts[i]
											.valueSparse(j) * heightRatio) > 0)
										g.drawLine(x, y, x, (int) (y + Math
												.round(m_histBarClassCounts[i]
														.valueSparse(j)
														* heightRatio)));
									g.setColor(Color.black);
								...
								// Drawing bar count on the top of the bar if it
								// is < barWidth
								if (m_fm.stringWidth(" "
										+ Integer.toString(sum)) < barWidth)
									g.drawString(" " + Integer.toString(sum),
											x, y - 1);
						...
						// Now drawing the axis line at the bottom of the
						// histogram
						...
						g.drawLine(x, this.getHeight() - 17,
								(barWidth == 1) ? x + barWidth
										* m_histBarClassCounts.length - 1 : x
										+ barWidth
										* m_histBarClassCounts.length,
								this.getHeight() - 17); // axis line -- see
														// footnote 2.
						g.drawLine(x, this.getHeight() - 16, x,
								this.getHeight() - 12); // minimum line
						g.drawString(
								Utils.doubleToString(m_as.numericStats.min, 2),
								x, this.getHeight() - 12 + m_fm.getHeight()); // minimum
																				// value
						g.drawLine(x + (barWidth * m_histBarClassCounts.length)
								/ 2, this.getHeight() - 16, x
								+ (barWidth * m_histBarClassCounts.length) / 2,
								this.getHeight() - 12); // median line
						// Drawing median value. X position for drawing the
						// value is: from
						// start of the plot take the mid point and subtract
						// from it half
						...
						g.drawString(
								...
								x
										...
										- m_fm.stringWidth(Utils
												.doubleToString(
														m_as.numericStats.max
																/ 2
																+ m_as.numericStats.min
																/ 2, 2)) / 2,
								this.getHeight() - 12 + m_fm.getHeight()); // median
																			// value
						g.drawLine((barWidth == 1) ? x + barWidth
								* m_histBarClassCounts.length - 1 : x
								+ barWidth * m_histBarClassCounts.length,
								this.getHeight() - 16, (barWidth == 1) ? x
										+ barWidth
										* m_histBarClassCounts.length - 1 : x
										+ barWidth
										* m_histBarClassCounts.length,
						...
						x = 3;
						if ((this.getWidth() - (x + m_histBarCounts.length
								* barWidth)) > 5)
							x += (this.getWidth() - (x + m_histBarCounts.length
									* barWidth)) / 2;
						...
						// Same as above
						for (int i = 0; i < m_histBarCounts.length; i++) {
							// calculating the ration of the component's height
							// compared to
							// the maxValue in our current attribute. Leaving 19
							// pixels to
							// draw the axis at the bottom of the histogram.
							heightRatio = (this.getHeight()
									- (float) m_fm.getHeight() - buttonHeight - 19)
									/ m_maxValue;
							y = this.getHeight()
									- Math.round(m_histBarCounts[i]
											* heightRatio) - 19;
								...
								g.drawLine(
										...
										y
												+ Math.round(m_histBarCounts[i]
														* heightRatio));
							if (m_fm.stringWidth(" "
									+ Integer.toString(m_histBarCounts[i])) < barWidth)
								g.drawString(
										" "
												+ Integer
														.toString(m_histBarCounts[i]),
										x, y - 1);
						...
						// This is exact the same as in the if stmt above. See
						// the inside of
						// the stmt for details
						g.drawLine(x, this.getHeight() - 17,
								(barWidth == 1) ? x + barWidth
										* m_histBarCounts.length - 1 : x
					...
					g.drawString(
							Messages.getInstance()
									.getString(
											"AttributeVisualizationPanel_PaintComponent_G_DrawString_Text_First"),
							this.getWidth()
									/ 2
									- m_fm.stringWidth(Messages
											.getInstance()
											.getString(
													"AttributeVisualizationPanel_PaintComponent_StringWidth_Text_First"))
									/ 2,
							this.getHeight() / 2 - m_fm.getHeight() / 2);
				}
			} // <--end if of calculation thread
			else if (m_displayCurrentAttribute) { // if still calculation thread
													// is running plot
				g.clearRect(0, 0, this.getWidth(), this.getHeight());
				g.drawString(
						Messages.getInstance()
								.getString(
										"AttributeVisualizationPanel_PaintComponent_G_DrawString_Text_Second"),
						this.getWidth()
								/ 2
								- m_fm.stringWidth(Messages
										.getInstance()
										.getString(
												"AttributeVisualizationPanel_PaintComponent_StringWidth_Text_Second"))
								/ 2, this.getHeight() / 2 - m_fm.getHeight()
								/ 2);
				...
				g.drawString(
						Messages.getInstance()
								.getString(
										"AttributeVisualizationPanel_PaintComponent_G_DrawString_Text_Third"),
						this.getWidth()
								/ 2
								- m_fm.stringWidth(Messages
										.getInstance()
										.getString(
												"AttributeVisualizationPanel_PaintComponent_StringWidth_Text_Third"))
								/ 2, this.getHeight() / 2 - m_fm.getHeight()
								/ 2);
			}
	...
	public static void main(String[] args) {
		if (args.length != 3) {
			final JFrame jf = new JFrame(Messages.getInstance().getString(
					"AttributeVisualizationPanel_Main_JFrame_Text"));
			ETWekaAttributeVisualizationPanel ap = new ETWekaAttributeVisualizationPanel();
			try {
				Instances ins = new Instances(new FileReader(args[0]));
				ap.setInstances(ins);
				System.out.println(Messages.getInstance().getString(
						"AttributeVisualizationPanel_Main_Text_First")
						+ args[0]
			...
			System.out.println(Messages.getInstance().getString(
					"AttributeVisualizationPanel_Main_Text_Fourth"));
			...
			System.out.println(Messages.getInstance().getString(
					"AttributeVisualizationPanel_Main_Text_Fifth"));
	}
}
