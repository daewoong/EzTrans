 ...
 */
public class ETWekaExplorer extends JPanel {
	...
	public static interface ExplorerPanel {
		...
		/**
		 * Sets the Explorer to use as parent frame (used for sending
	...
	protected ETWekaPreprocessPanel m_PreprocessPanel = new ETWekaPreprocessPanel();
	...
	public ETWekaExplorer() {
		...
		String[] tabs = ETWekaExplorerDefaults.getTabs();
		...
		for (int i = 0; i < m_Panels.size(); i++) {
			HashSet options = tabOptions.get(m_Panels.get(i).getClass()
					.getName());
		...
		m_PreprocessPanel
				.addPropertyChangeListener(new PropertyChangeListener() {
					public void propertyChange(PropertyChangeEvent e) {
						for (int i = 0; i < m_Panels.size(); i++) {
							m_Panels.get(i).setInstances(
									m_PreprocessPanel.getInstances());
	 ...
	 * @return the panel
	 */
	public ETWekaPreprocessPanel getPreprocessPanel() {
	...
	/**
	 * Removes the specified listener from the set of listeners if it is
	 * present.
	 * 
	 * @param l
	 ...
	 */
	private static ETWekaExplorer m_explorer;
	...
	public static void main(String[] args) {
		...
		weka.core.logging.Logger.log(weka.core.logging.Logger.Level.INFO,
				Messages.getInstance().getString("Explorer_Main_Logger_Text"));
		...
		try {
			...
			m_explorer = new ETWekaExplorer();
			jf = new JFrame("데이터마이닝 - Weka");
			jf.getContentPane().setLayout(new BorderLayout());
			jf.getContentPane().add(m_explorer, BorderLayout.CENTER);
			jf.addWindowListener(new WindowAdapter() {
				public void windowClosing(WindowEvent e) {
					System.out.println("mining close");
					jf.dispose();
					try {
